...
mesh_folder="../mesh/bar/unstructed"
...
result_folder="/home/yiwei/results_new/bar3d_dyn"
...
def run_program(solver_,mesh_,j_file_,discr_order_,n_ref_,block_size_,repeat_time_,num_thread_,dt_):
    temp_path=os.path.join(result_folder,"dt"+str(dt_),solver_,os.path.splitext(os.path.basename(mesh_))[0],os.path.splitext(os.path.basename(j_file_))[0],discr_order_name[discr_order_-1],"ref"+str(n_ref_),"block"+str(block_size_),"Thread"+str(num_thread_),str(repeat_time_))
    ...
    # json_data["output"]["json"] ="result.json"
                                ...
                                for num_thread in num_threads:
                                    for repeat_time in range(repeat_times):
                                        if (block_size==1) or (block_enable):
                                            ...
                                            print(solver+"_"+mesh_name+"_"+discr_order_name[discr_order-1]+"_"+"n_ref"+str(n_ref)+"_"+"Block"+str(block_size)+"dt"+str(dt))
                                            run_program(solver,mesh_file,json_file,discr_order,n_ref,block_size,repeat_time,num_thread,dt)
